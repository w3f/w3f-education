...
With all of our tools in place to register a user, now let's go through the process of properly registering them via an **extrinsic**, or state change.

:::info What is an extrinsic again?

An extrinsic is a transaction, or a unit to define how state should change within the network.
...
We are going to create a function called `register`, which will take a number of parameters and perform the following:

- Take a parameter, `name`, of the user.
- Take a parameter, `bio`, of the user.
...
With our requirements properly defined, we can begin coding this function.  Go ahead and paste the function called `register` that includes some beginning logic to start:
...
A very useful macro, `ensure!`, is provided by FRAME.  This macro enables us to check a condition, and if it doesn't check out, allow an extrinsic to fail with a specific error.  

We can also use the `Currency` trait that is included with our configuration.  We will elaborate more on this trait later, but know that for now, it enables us to check the balance of the sender:

```rust
// Retrieve "free" balance of user
...
If you notice, the parameters (`name` and `bio`) provided in `register` are of type `Vec<u8>`.  This is a vector of bytes, which for now, you may think of as a String.  

Within our config, we have two notable constants which are defined: `MaxBioLength` and `MaxNameLength`.  We want our two parameters to be **bounded** to these limits, as we shouldn't allow for infinite values to be stored on chain.
...
### Generate gradient profile and build our user

Once we have our parameters converted and ready, we can build our user.

Firstly, we can call another trait, `T::Randomness`, to provide a random value to the included `generate_hex_values` function.  

This will return two randomly generated hex values that can be used to generate a gradient profile picture:

```rust
// Generate our random profile picture (aka, two hex values which form a gradient)
...
// Construct our UserMetadata.  Ideally, we could also create an implemention to make
...
With our user fully configured, we can now lock their balance and finish the registration process by storing them in our `RegisteredUsers` mapping:
...
// Store the user, add to existing names, and update total amount of users
...
Once we registered the user, we can now also update the total number of users on the network.  Note the use of `checked_add` and `unwrap_or_default()`:
