...
In fact, *any* type that `impl`s `Transferrable` can now transfer funds, so long as it's implemented correctly as per the method and trait signatures.
...
And as before, it's as simple as calling the `drain_funds` on any type, in this case, `Account`, that implements the `Transferrable`trait:
